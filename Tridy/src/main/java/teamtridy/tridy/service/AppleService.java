...
public class AppleService {
    ...
    private PublicKey getPublicKey(String idToken) {
        try {
            ...
            ResponseEntity<ApplePublicKeyResponseDto> response = restTemplate.exchange(appleUrlPublicKeys, HttpMethod.GET, request, ApplePublicKeyResponseDto.class);

            if (response.getStatusCode() == HttpStatus.OK) {
                //get jwt token header
                String headerOfIdentityToken = idToken.substring(0, idToken.indexOf(".")); //jwt은 우선  header와 payload, signature로 구성되어있습니다. header에는 kid(key ID), alg(알고리즘 유형)으로 구성되어있고 payload는 전달하고자 하는 정보들이 담겨있습니다. signature는 header와 payload를 비밀키와 header에 있는 알고리즘 유형으로 암호화한 정보입니다.
                ...
                ApplePublicKeyResponseDto.Key key = response.getBody().getMatchedKeyBy(header.get("kid"), header.get("alg"))
