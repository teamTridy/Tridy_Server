...
public interface PlaceRepository extends JpaRepository<Place, Long> {
    ...
    @Query("SELECT p FROM Place p " +
            ...
            "ORDER BY (CASE WHEN p.name LIKE %:query% THEN 1 WHEN p.intro LIKE %:query% THEN 2 ELSE 3 END), p.id")
    Slice<Place> findAllByQueryAndLocationIn(String query, List<Location> locations, Pageable pageable);

    @Query("SELECT p FROM Place p " +
            "WHERE p.category in :categories AND (p.name LIKE %:query% OR p.intro LIKE %:query% OR p.story LIKE %:query%) " +
            "ORDER BY (CASE WHEN p.name LIKE %:query% THEN 1 WHEN p.intro LIKE %:query% THEN 2 ELSE 3 END), p.id")
    Slice<Place> findAllByQueryAndCategoryIn(String query, List<Category> categories, Pageable pageable);
    ...
    @Query("SELECT p FROM Place p " +
            ...
            "ORDER BY (CASE WHEN p.name LIKE %:query% THEN 1 WHEN p.intro LIKE %:query% THEN 2 ELSE 3 END), p.id")
    Slice<Place> findAllByQueryAndCategoryInAndLocationIn(String query, List<Category> categories ,List<Location> locations, Pageable pageable);
